...
export function buildDynamicLongRunStructure(params: {
  ...
  if (phase === "BUILD") {
    ...
    if (styleIdx === 0) {
      ...
      return {
        ...
        powerTarget: "72% CP Z2 -> Aceleración final 82% CP",
        workoutDoc: `Warmup\n- 15m 68% FTP\n\nMain (Base Aeróbica)\n- ${Math.max(10, easyMins)}m 73% FTP\n\nFast-Finish (Ritmo Específico)\n- ${fastMins}m 82% FTP\n\nCooldown\n- 10m 60% FTP`,
      };
    }
    if (styleIdx === 1) {
      ...
      return {
        workoutName: `Tirada Larga con Bloques de Ritmo Específico (${baseKm} km con 2x${blockMins}m @ 82% CP)`,
        powerTarget: "82% CP en bloques de ritmo",
        workoutDoc: `Warmup\n- 15m 68% FTP\n\nMain (Z2)\n- ${Math.max(10, Math.round(baseSub / 2))}m 72% FTP\n\n2x\n- ${blockMins}m 82% FTP\n- 5m 68% FTP\n\nMain (Z2)\n- ${Math.max(10, Math.round(baseSub / 2))}m 72% FTP\n\nCooldown\n- 10m 60% FTP`,
      };
    }
    ...
    return {
      ...
      workoutDoc: `Warmup\n- 15m 68% FTP\n\nMain (Z2)\n- ${Math.max(15, baseMins - 55)}m 73% FTP\n\n3x (Flotaciones Dinámicas)\n- 5m 80% FTP\n- 3m 68% FTP\n\nCooldown\n- 10m 60% FTP`,
    };
  }
  ...
  if (weekNumber % 2 === 0) {
    ...
    return {
      ...
      workoutDoc: `Warmup\n- 15m 66% FTP\n\nMain (Z2 Cómoda)\n- ${Math.max(15, baseMins - finalProgMins - 25)}m 72% FTP\n\nFinal Ágil\n- ${finalProgMins}m 76% FTP\n\nCooldown\n- 10m 60% FTP`,
    };
  }

  return {
    workoutName: `Tirada Larga de Construcción Aeróbica Z2 (${baseKm} km / ${baseMins}m)`,
    powerTarget: "70-74% CP (Z2 Base)",
    workoutDoc: `Warmup\n- 15m 66% FTP\n\nMain (Z2 Cómoda)\n- ${Math.max(10, baseMins - 25)}m 72% FTP\n\nCooldown\n- 10m 60% FTP`,
  };
}
