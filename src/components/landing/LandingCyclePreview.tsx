    ...
    <section id="ciclos" className="py-20 px-4 sm:px-6 max-w-7xl mx-auto w-full">
      <div className="text-center max-w-3xl mx-auto mb-14 space-y-3">
        ...
        <p className="text-sm sm:text-base text-slate-600 leading-relaxed font-normal">
          Observa la periodización de tu temporada y cómo se traducen los bloques científicos en perfiles de potencia para tu reloj.
        </p>
      </div>

      {/* 1. Las 4 Fases de Periodización Progresiva */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-12">
        <div className="bg-white/90 border border-slate-200/90 rounded-3xl p-6 shadow-sm backdrop-blur-xl hover:border-cyan-400 transition-all">
          ...
          <p className="text-xs text-slate-600 mt-3 leading-relaxed">
            Construcción de densidad capilar en Zona 2 suave, fortaleciendo tendones y sóleo sin fatiga neural.
          </p>
          <div className="mt-4 text-[10px] font-mono font-bold text-cyan-800 bg-cyan-50 px-2.5 py-1 rounded-lg inline-block border border-cyan-200/60">
            Sem 1 - 4 • Carga 3:1
          </div>
        </div>

        <div className="bg-white/90 border border-slate-200/90 rounded-3xl p-6 shadow-sm backdrop-blur-xl hover:border-amber-400 transition-all">
          ...
          <p className="text-xs text-slate-600 mt-3 leading-relaxed">
            Bloques de umbral anaeróbico y Sweetspot para elevar tu potencia de crucero y resistir ritmos exigentes.
          </p>
          <div className="mt-4 text-[10px] font-mono font-bold text-amber-800 bg-amber-50 px-2.5 py-1 rounded-lg inline-block border border-amber-200/60">
            Sem 5 - 10 • Umbral Lactato
          </div>
        </div>

        <div className="bg-white/90 border border-slate-200/90 rounded-3xl p-6 shadow-sm backdrop-blur-xl hover:border-rose-400 transition-all">
          ...
          <p className="text-xs text-slate-600 mt-3 leading-relaxed">
            Tiradas cumbre con fracciones prolongadas a potencia exacta de carrera para dominar el ritmo del día D.
          </p>
          <div className="mt-4 text-[10px] font-mono font-bold text-rose-800 bg-rose-50 px-2.5 py-1 rounded-lg inline-block border border-rose-200/60">
            Sem 11 - 13 • Fondo Cumbre
          </div>
        </div>

        <div className="bg-white/90 border border-slate-200/90 rounded-3xl p-6 shadow-sm backdrop-blur-xl hover:border-emerald-400 transition-all">
          ...
          <p className="text-xs text-slate-600 mt-3 leading-relaxed">
            Descarga exponencial de fatiga (-50%) conservando toques de intensidad para llegar supercompensado.
          </p>
          <div className="mt-4 text-[10px] font-mono font-bold text-emerald-800 bg-emerald-50 px-2.5 py-1 rounded-lg inline-block border border-emerald-200/60">
            Sem 14 - 16 • Supercompensación
          </div>
        </div>
      </div>

      {/* 2. Visualizador Interactivo con WorkoutChart */}
      <div className="bg-white/90 border border-slate-200/90 rounded-3xl p-6 sm:p-8 shadow-xl shadow-slate-200/30 backdrop-blur-xl space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-100 pb-5">
          ...
          <div className="flex flex-wrap items-center gap-2">
            <span className="flex items-center gap-1 px-3 py-1.5 rounded-xl bg-slate-100 font-mono text-xs font-bold text-slate-800 border border-slate-200">
              <Clock className="h-3.5 w-3.5 text-slate-500" />
              <span>{currentWorkout.duration}</span>
            </span>
            <span className="flex items-center gap-1 px-3 py-1.5 rounded-xl bg-amber-50 font-mono text-xs font-bold text-amber-800 border border-amber-200">
              <Zap className="h-3.5 w-3.5 text-amber-600" />
              <span>{currentWorkout.powerTarget}</span>
            </span>
            <span className="flex items-center gap-1 px-3 py-1.5 rounded-xl bg-emerald-50 font-mono text-xs font-bold text-emerald-800 border border-emerald-200">
              <Target className="h-3.5 w-3.5 text-emerald-600" />
              <span>~{currentWorkout.tss} TSS</span>
            </span>
          </div>
        </div>
        ...
        {/* Selector de Sesiones de Muestra */}
        <div className="pt-2">
          <div className="text-xs font-bold text-slate-700 mb-3">
            Selecciona una sesión estructurada para ver su gráfica:
          </div>
