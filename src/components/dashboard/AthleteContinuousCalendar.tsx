...
export const AthleteContinuousCalendar: React.FC<AthleteContinuousCalendarProps> = ({
  ...
  return (
    <div className="space-y-4 animate-fadeIn select-none overflow-x-auto">
      {/* 1. CABECERA GLOBAL DE DÍAS (1:1 MATEMÁTICA) */}
      <div className={`min-w-[1100px] grid ${gridTemplate} gap-2.5 px-2.5 text-center font-mono text-[11px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider pb-2 border-b border-slate-200 dark:border-slate-800`}>
      ...
      {/* 2. FILAS SEMANALES EN CUADRÍCULA DE 8 COLUMNAS */}
      <div className="min-w-[1100px] space-y-4">
        {weeks.map((week, wIdx) => {
          const isCurrentWeek = week.startDate === currentMonStr || (blueprint.currentWeekIndex !== undefined && wIdx === blueprint.currentWeekIndex);
          ...
          const effectiveExecuted = executedDirectTotalTss > 0
            ? executedDirectTotalTss
            : (isCurrentWeek ? weeklyExecutedTss : (isPastWeek ? plannedTss : 0));
          ...
          return (
            ...
            >
              <div className={`grid ${gridTemplate} gap-2.5 items-stretch`}>
                    ...
                    <div className="flex items-center justify-start border-b border-slate-200 dark:border-slate-800 pb-1.5">
                      <div className="flex items-center space-x-1.5">
                        <span className="font-black text-slate-900 dark:text-white text-sm">
                          Sem. {week.weekNumber} <span className="text-slate-400 font-normal text-[11px]">/ {weeks.length}</span>
                        </span>
                        <span className="px-1.5 py-0.2 rounded bg-slate-200 dark:bg-slate-800 text-[10px] font-mono text-slate-600 dark:text-slate-400 font-bold" title="Semana del año">
                        ...
                        <div className="space-y-0.5">
                          ...
                          <div className="h-1.5 w-full bg-slate-200 dark:bg-slate-800 rounded-full overflow-hidden">
                            <div
                              ...
                              style={{ width: `${bikeTss > 0 ? Math.min(100, Math.round(((executedBikeTss || (isPastWeek ? bikeTss : 0)) / bikeTss) * 100)) : 0}%` }}
                            />
                          </div>
                        </div>
                      )}
                        ...
                        <div className="space-y-0.5">
                          ...
                          <div className="h-1.5 w-full bg-slate-200 dark:bg-slate-800 rounded-full overflow-hidden">
                            <div
                              ...
                              style={{ width: `${runTss > 0 ? Math.min(100, Math.round(((executedRunTss || (isPastWeek ? runTss : 0)) / runTss) * 100)) : 0}%` }}
                            />
                          </div>
                        </div>
                      )}
                ...
                {/* COLUMNAS 2 A 8: 7 DÍAS CANÓNICOS CON SOPORTE PARA DOBLES SESIONES APILADAS */}
                {(() => {
