...
import { X, Mail, Lock, User, Eye, EyeOff, Sparkles, ArrowRight, ShieldCheck, AlertCircle } from "lucide-react";
...
export const AuthModal: React.FC<AuthModalProps> = ({ isOpen, onClose, initialTab = "login" }) => {
  ...
  const handleEmailSubmit = async (e: React.FormEvent) => {
    ...
    if (activeTab === "register") {
      ...
      if (!displayName.trim()) {
        setLocalError("Ingresa tu nombre y apellidos.");
        return;
      }
      ...
      <div className="w-full max-w-md bg-white rounded-3xl p-6 sm:p-7 border border-slate-200/90 shadow-2xl relative space-y-4 text-slate-900 max-h-[92vh] overflow-y-auto">
        ...
        <div className="text-center space-y-1 pt-1">
          ...
          <p className="text-xs text-slate-500">
            {activeTab === "login" && "Accede a tu plan de entrenamiento y analítica."}
            {activeTab === "register" && "Regístrate para solicitar acceso a tus planes guiados por IA."}
        ...
        {activeTab !== "forgot" && (
          <div className="space-y-3">
            ...
            <div className="relative flex py-1 items-center">
              ...
              <span className="flex-shrink mx-3 text-[11px] font-bold text-slate-400 uppercase tracking-wider">o con tu correo</span>
              <div className="flex-grow border-t border-slate-200"></div>
            </div>
          </div>
        )}
        ...
        <form onSubmit={handleEmailSubmit} className="space-y-3">
          ...
          {activeTab === "register" && (
            <div>
              <label className="block text-[11px] font-bold text-slate-700 uppercase tracking-wider mb-1">Confirmar Contraseña</label>
              <div className="relative">
                <Lock className="absolute left-3 top-2.5 h-4 w-4 text-slate-400" />
                <input
                  type={showPassword ? "text" : "password"}
          ...
          {activeTab === "register" && (
            <p className="text-[11px] text-slate-500 bg-slate-50 p-2.5 rounded-xl border border-slate-100 leading-relaxed">
              💡 <strong>Nota para atletas:</strong> Si tu entrenador ya te invitó con este correo, ingresarás de inmediato. De lo contrario, tu solicitud quedará en cola para su activación.
            </p>
          )}
          ...
          >
            {isSubmitting ? (
              ...
              <>
                <span>Enviar Solicitud / Registrarme</span>
