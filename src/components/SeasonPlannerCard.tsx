...
import { Trophy, Calendar, Target, Flag, ArrowRight, Settings2, Sparkles } from "lucide-react";
        ...
        <div className="flex items-center space-x-2.5">
          ...
          <div>
            <h2 className="text-sm font-bold text-white tracking-wide flex items-center gap-2">
              Macrociclo & Calendario de Temporada
            </h2>
            <p className="text-[11px] text-slate-400">
              Periodización biológica adaptada a tus competiciones objetivo
            </p>
          </div>
        </div>
      ...
      {primaryRace ? (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          ...
          <div className="md:col-span-2 rounded-xl bg-slate-950/80 p-4 border border-slate-800/80 flex flex-col justify-between space-y-2">
            ...
            {/* Guideline */}
            {phaseInfo && (
              <div className="rounded-lg bg-slate-900/90 p-2.5 border border-slate-800 text-[11px] text-slate-300 leading-relaxed">
                <strong className="text-amber-300">Pauta de Fase: </strong>
                {phaseInfo.guideline}
              </div>
            )}
          </div>
          ...
          <div className="rounded-xl bg-gradient-to-br from-slate-950 via-slate-900 to-amber-950/20 p-4 border border-slate-800/80 flex flex-col justify-between">
            ...
            <div className="mt-3 border-t border-slate-800 pt-2">
              <span className="text-[10px] uppercase font-bold text-slate-400">Enfoque Semanal:</span>
              <p className="text-xs font-bold text-emerald-400 mt-0.5">
                {phaseInfo?.suggestedFocus}
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="rounded-xl bg-slate-950/60 p-5 border border-slate-800 text-center space-y-2">
          ...
          <p className="text-[11px] text-slate-400 max-w-md mx-auto">
            Agrega tu próxima competición objetivo (ej. Maratón, Media Maratón o Fondo) para que el agente calcule automáticamente las fases de tu macrociclo y periodice tus cargas.
          </p>
          <button
            onClick={onOpenRaceSettings}
