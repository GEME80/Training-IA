...
export default function Error({
  ...
  useEffect(() => {
    console.error("App Router Error Boundary capturó un error:", error);
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-4 text-center">
      <div className="max-w-md w-full bg-white p-6 rounded-2xl shadow-xl border border-slate-200 space-y-4">
        ...
        <h2 className="text-lg font-bold text-slate-900">
          {isChunkError ? "Sincronizando con el Servidor" : "Algo no salió como se esperaba"}
        </h2>
        <p className="text-xs text-slate-600 font-mono bg-slate-100 p-3 rounded-lg text-left break-words">
          {isChunkError
            ...
            : error?.message || "Error inesperado al cargar la vista"}
        </p>
        <button
          type="button"
        ...
        >
          {isChunkError ? "Actualizar Ahora" : "Reintentar Carga"}
        </button>
      </div>
    </div>
  );
